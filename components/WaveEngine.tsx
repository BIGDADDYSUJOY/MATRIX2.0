...
const WaveEngine: React.FC<WaveEngineProps> = ({ waveState }) => {
  ...
  // Sync ref with prop to decouple animation loop from React re-renders
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    const ctx = canvas.getContext('2d', { alpha: false }); // Optimization: Solid background
    if (!ctx) return;
    ...
    const draw = () => {
      // Optimization: use fillRect instead of clearRect for solid backgrounds
      ctx.fillStyle = '#000';
      ...
      // Draw Grid - Optimization: Batch all lines into a single stroke call
      ...
      // Optimization: Hoist invariant calculations out of the loop
  ...
  }, []); // Empty dependency array ensures loop isn't restarted on state changes
